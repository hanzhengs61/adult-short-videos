          ...
          <div v-if="userStore.authModal.show"
            ...
            <div class="px-6 py-5">
              ...
              <form v-if="userStore.authModal.mode === 'login'" @submit.prevent="handleLogin" class="space-y-3">
                ...
                <div v-if="errorMsg" class="text-xs text-red-400 bg-red-400/10 border border-red-400/20 rounded-lg px-3 py-2">
                  {{ errorMsg }}
                </div>
...
.modal-panel-enter-active { transition: all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1); }
.modal-panel-leave-active  { transition: all 0.2s ease; }
.modal-panel-enter-from    { opacity: 0; transform: translateY(40px) scale(0.97); }
.modal-panel-leave-to      { opacity: 0; transform: translateY(20px); }
@media (max-width: 640px) {
  .modal-panel-enter-from { transform: translateY(100%); }
  .modal-panel-leave-to   { transform: translateY(100%); }
}
</style>
