  ...
  <div class="max-w-screen-2xl mx-auto px-3 sm:px-4 py-4">
    ...
    <div v-if="!userStore.isLoggedIn"
      ...
      <div class="text-center">
        <p class="text-text-primary font-bold text-2xl mb-1">关注喜欢的创作者</p>
        <p class="text-text-primary/60 text-base">登录后可关注，第一时间收到最新视频</p>
      </div>
      ...
      <div class="mb-6">
        ...
        <div v-if="!following.length"
          ...
          <div class="w-12 h-12 rounded-full bg-bg-hover flex items-center justify-center shrink-0">
            <svg class="w-6 h-6 text-text-primary/60" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/>
            </svg>
          </div>
          <div>
            <p class="text-text-primary text-base font-medium">还没有关注任何人</p>
            <p class="text-text-primary/60 text-sm mt-0.5">去探索页找到感兴趣的创作者</p>
          </div>
        ...
        <div v-else class="flex gap-3 overflow-x-auto scrollbar-none pb-1">
          <div v-for="u in following" :key="u.id"
            ...
            <span class="text-xs text-text-primary/60 group-hover:text-primary transition-colors
